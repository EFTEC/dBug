...
echo "<hr> with variables types:";
new dBug(new Vegetable("spinach"));
echo "<hr> without variables types:";
...
//http://php.net/manual/en/function.imagesetpixel.php
{
	...
	for ($i = 0; $i < 10000; $i++) {
		...
		$g = abs(255 * (1 - $y / 190));
		$g = ($g > 255) ? 255 : $g;
		imagesetpixel($img, round($x), round($y), imagecolorallocate($img, $r, $g, 255 * $a / 2));
		$x = ($x + $corners[$a]['x']) / 2;
		$y = ($y + $corners[$a]['y']) / 2;
	}
	new dBug($img);
